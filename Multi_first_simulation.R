## 2-channel bootstrap simulation steps of data D:
...
rep1=1000
check<-c()
distribution1_2<- function(psi,data2){
  for (ii in 1:rep1){
    for (i in 1:2) {
      y1<-data2[i,1]
      y2<-data2[i,2]
      ...
      y1 <- rpois(1,gamma_hat[i]*psi+beta_hat[i])
      y2 <- rpois(1,beta_hat[i]*t)
      y3 <- rpois(1,gamma_hat[i]*u)
      c(y1,y2,y3)
      if(y1==0&y2==0&y3==0){
        D[i,1:3]<-c(rpois(1,gamma_hat[i]*psi+beta_hat[i])+1,rpois(1,beta_hat[i]*t),rpois(1,gamma_hat[i]*u))
      } else{
        D[i,1:3]<-c(y1,y2,y3)
      }
    }
    ...
    if ((log_psihat-logpsi) < 0  ){
      ...
      r1[ii]<-sign(psi_hat-psi)*sqrt(2*(log_psihat-logpsi))
    }
  }
  return(r1)
}
...
r1<-distribution1_2(0,data2)
which(is.na(r1)) 

plot(density(r1),main='simulated distribution of r(0) of 2-channel')
#pvalue of the simulation distribution of 2-channel:
1-sum(r1 < r_root2(0,data2))/rep1  #
##############################
#plot the significance function:
s1<-c()
significance_21<-function(psi){
  for (i in 1:length(psi)){
    s1[i]<- sum(distribution1_2(psi[i],data2) < r_root2(psi[i],data2))/rep
  }
  return(s1)
}

s1<-significance_21(seq(-3,50,1))


###### first coverage for two channel #######
...
R_<-c()
rep=10000
y<-matrix(data=0,nrow=rep,ncol=3*2)
for (i in 1:rep){
  for (ii in 1:2){
  ...
  y[i,]<-c(y1,y2,y3)
  R_<-distribution1_2(psi,data2)
  p11[i] <- sum(R_< r_root2(psi,data2))/rep1
}
