...
CI22 <- function(alpha){
  ...
  for (i in 1:100){
    for (ii in 1:2) {
      ...
      gg[ii]<-gammahat[ii]*psi_j[i]+betahat[ii]
      if (gg[ii]<0){
        gg[ii]<-0
      }
      Y1 <- rpois(1,gg)
      ...
      c(Y1,Y2,Y3)
      if(Y1==0&Y2==0&Y3==0){
        D[ii,1:3]<-c(rpois(1,gg)+1,rpois(1,betahat[ii]*t),rpois(1,gammahat[ii]*u))
      } else{
        D[ii,1:3]<-c(Y1,Y2,Y3)
      }
    }
    ...
    if ((log_psihat-logpsi) < 0  ){
      ...
      r2<-sign(psi_hat-psi_j[i])*sqrt(2*(log_psihat-logpsi))
    }

    if ( r2 <= r_root2(psi_j[i],data2)  ){
...
CI_plot2<-c()
ite=50
CI_average2<-matrix(NA,ite,2)

for (i in 1:ite){
  CI_average2[i,]<-colMeans(Width_CI22(0.01))
