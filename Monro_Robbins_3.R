#Monro Robbins for the third method:
psi_ <- seq(-3,50,0.001)
alpha <- 0.01
x <- psi_
####Calculate the CI based on third method###############
diff<-c()
CI3 <- function(alpha){
  ...
  for (i in 1:100){
    ...
    if ((log_psihat-logpsi)<0){
      ...
      r3<-sign(psi_hat-psihat)*sqrt(2*(log_psihat-logpsi))
    }
    
    if ( r3 <= r_root(psi_j[i],y1,y2,y3)  ){
...
###############Average the CI with 10 replications based on third method################################
rep=10
CI_mat3 <- matrix(data = NA, nrow = rep, ncol = 2)
Width_CI3<- function(alpha){
  for (i in 1:rep){
    result_L<-CI3(alpha)
    index_L<-which(abs(result_L[[2]] - 0.0001) == sort(abs(result_L[[2]] - 0.0001), decreasing = FALSE)[1])
    LL<-result_L[[1]][index_L]
    
    result_U<-CI3(1-alpha)
...
limits3<-Width_CI3(0.01)
colMeans(limits3)
CI_average3<-colMeans(limits3)
###########Check third simulation gives shorter CIs###############################################
CI_plot3<-c()
CI_average3<-matrix(NA,50,2)
for (i in 1:50){
  CI_average3[i,]<-colMeans(Width_CI3(0.01))
